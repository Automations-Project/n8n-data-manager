...
### Added (v5.1 CLI Frontend UX — in progress)

- Internal Bash/Gum UI contract helpers in `src/lib/ui_contracts.sh` for consistent sections, status rows, tables, risk blocks, previews, confirmations, and recovery output.
- Central plain-mode and promptability guards so enhanced UI paths degrade safely when `HAS_GUM=false`, color is disabled, stdout is piped, stdin/stdout are non-TTY, or `CI`/`GITHUB_ACTIONS` are set.
- Strict plain layout fallback across `src/lib/gum_ui.sh` and `src/lib/gum_layouts.sh` — no pagers, spinners, ANSI color, emoji, Unicode borders, or `column` dependencies in plain mode.
- Legacy Gum wrapper hardening: prompt-capable wrappers refuse before reading stdin in non-interactive contexts and return `1` for cancellation / `2` for non-interactive refusal.
- Regression test suites: `tests/regression/ui-plain-mode.bats`, `tests/regression/ui-legacy-wrapper-guards.bats`, `tests/regression/ui-scriptability.bats`, and `tests/regression/gum-fallback.bats`.
...
- **Modular architecture**: Bash + Gum CLI (single compiled `n8n-manager` binary)
